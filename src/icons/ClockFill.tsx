import { forwardRef } from 'react'
import type { SVGProps } from 'react'

export interface ClockFillIconProps extends SVGProps<SVGSVGElement> {
  size?: number | string
}

export const ClockFillIcon = forwardRef<SVGSVGElement, ClockFillIconProps>(
  ({ size = 32, ...props }, ref) => (
    <svg
      ref={ref}
      fillRule="evenodd"
      clipRule="evenodd"
      strokeLinejoin="round"
      strokeMiterlimit={1.414}
      xmlns="http://www.w3.org/2000/svg"
      aria-label="ClockFill"
      viewBox="0 0 32 32"
      preserveAspectRatio="xMidYMid meet"
      fill="currentColor"
      width={size}
      height={size}
      {...props}
    >
      <path d="M28 16c0 6.627-5.373 12-12 12S4 22.627 4 16 9.373 4 16 4s12 5.373 12 12zm-13.34.201a.997.997 0 0 1-.02-.201V9a1 1 0 0 1 2 0v6.425l4.696 2.711a1 1 0 0 1-1 1.732l-5.196-3a.998.998 0 0 1-.48-.667z" />
    </svg>
  ),
)

ClockFillIcon.displayName = 'ClockFillIcon'
