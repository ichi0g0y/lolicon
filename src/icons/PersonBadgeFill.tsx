import { forwardRef } from 'react'
import type { SVGProps } from 'react'

export interface PersonBadgeFillIconProps extends SVGProps<SVGSVGElement> {
  size?: number | string
}

export const PersonBadgeFillIcon = forwardRef<SVGSVGElement, PersonBadgeFillIconProps>(
  ({ size = 32, ...props }, ref) => (
    <svg
      ref={ref}
      fillRule="evenodd"
      clipRule="evenodd"
      strokeLinejoin="round"
      strokeMiterlimit={1.414}
      xmlns="http://www.w3.org/2000/svg"
      aria-label="PersonBadgeFill"
      viewBox="0 0 32 32"
      preserveAspectRatio="xMidYMid meet"
      fill="currentColor"
      width={size}
      height={size}
      {...props}
    >
      <path d="M26 16C26 5 25.167 4 16 4C6.833 4 6 5 6 16C6 27 6.833 28 16 28C25.167 28 26 27 26 16ZM19 16C19 17.683 18.729 18.241 18.531 18.456C18.368 18.632 17.851 19 16 19C14.15 19 13.633 18.632 13.47 18.456C13.272 18.241 13 17.683 13 16C13 14.343 14.343 13 16 13C17.657 13 19 14.343 19 16ZM19.835 19.977C20.714 19.173 21 17.873 21 16C21 13.239 18.762 11 16 11C13.239 11 11 13.239 11 16C11 17.873 11.287 19.173 12.166 19.977C10.501 20.888 9.19601 22.373 8.51701 24.166C8.39301 24.494 8.36301 24.874 8.56801 25.159C9.13701 25.948 10.242 25.048 10.698 24.189C11.706 22.292 13.702 21 16 21C18.298 21 20.295 22.292 21.303 24.189C21.759 25.048 22.864 25.948 23.432 25.159C23.637 24.874 23.608 24.494 23.484 24.166C22.804 22.373 21.499 20.888 19.835 19.977ZM14 6C13.448 6 13 6.448 13 7C13 7.552 13.448 8 14 8H18C18.552 8 19 7.552 19 7C19 6.448 18.552 6 18 6H14Z" />
    </svg>
  ),
)

PersonBadgeFillIcon.displayName = 'PersonBadgeFillIcon'
